...
iree_hal_driver_t *HostCPUSystemBuilder::InitializeHostCPUDriver(System &lsys) {
  ...
  auto distribution_option = config_options().GetBool("SHORTFIN_HOSTCPU_TOPOLOGY_DISTRIBUTION_SCATTER", true) ;
  iree_task_topology_distribution_t distribution = static_cast<iree_task_topology_distribution_t>(distribution_option) ;
  ...
  for (auto node_id : selected_nodes) {
    ...
    SHORTFIN_THROW_IF_ERROR(iree_task_topology_initialize_from_physical_cores(
        node_id, performance_level, distribution, *max_group_count, &topology.topology));
