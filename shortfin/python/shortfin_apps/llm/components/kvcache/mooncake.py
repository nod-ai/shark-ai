    ...
    def from_json(json_file: str) -> 'MooncakeConfig':
        ...
        try:
          config = MooncakeConfig(
            ...
            global_segment_size=config_data.get('global_segment_size', 1024 * 1024 * 1024 * 3),  # 3GB
            local_buffer_size=config_data.get('local_buffer_size', 1024 * 1024 * 1024),  # 1GB
            protocal=config_data.get('protocal', 'tcp'),
            device_name=config_data.get('device_name', ''),
            master_server_address=config_data.get('master_server_address', 'localhost:8080')
          )
...
class MooncakeEngine:
  ...
  def __init__(self, config: MooncakeConfig):
    ...
    try:
      ...
      raise ImportError("Mooncake is not installed. "
                        "Please run "
                        "pip3 install mooncake-transfer-engine "
                        "to install the Mooncake package.") from e
    self.engine = TransferEngine()
    self.config = config
    self.engine.initialize_ext(config.local_hostname,
    ...
    def __init__(self, config: MooncakeConfig):
        ...
        try:
          ...
          self.store.setup(self.config.local_hostname,
            ...
            raise RuntimeError("Failed to initialize MooncakeStore. "
                               "Please check your Mooncake configuration.") from e
