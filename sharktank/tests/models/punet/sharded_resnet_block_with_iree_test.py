...
def run_test_toy_size_sharded_resnet_block_with_iree(artifacts_dir: Path):
    ...
    iree_args = prepare_iree_module_function_args(args=input_args, devices=iree_devices)
    ...
    iree_results = [
        ops.to(iree_results[i], dtype=expected_results[i].dtype)
        for i in range(len(expected_results))
    ]

    # Observed atol is 1e-5.
    torch.testing.assert_close(iree_results, expected_results, rtol=0, atol=5e-5)
