...
class PagedKVCacheTest(unittest.TestCase):
    ...
    def test_paged(self):
        ...
        write_seq_length = self.seq_length - self.block_seq_stride
        page_count = self.bs * self.seq_length // self.block_seq_stride
        page_ids = torch.arange(page_count, dtype=torch.int64)
        page_ids = page_ids.view(self.bs, self.seq_length // self.block_seq_stride)
        write_page_ids = page_ids[:, : write_seq_length // self.block_seq_stride]
        ...
        # Write a prefill in:
        shape = self.bs, write_seq_length, self.attn_head_count, self.attn_head_dim
        ...
        for i in range(self.transformer_block_count):
            ...
            assert_tensor_close(
                read_ones[0], torch.full(read_ones[0].shape, 0.0).to(dtype=self.dtype)
            )
            assert_tensor_close(
                read_ones[1], torch.full(read_ones[0].shape, 0.0).to(dtype=self.dtype)
            )

        # Write timestep
        ts_shape = (self.bs, 1, self.attn_head_count, self.attn_head_dim)
        write_threes = torch.rand(*ts_shape).to(dtype=self.dtype)
        write_fours = torch.rand(*ts_shape).to(dtype=self.dtype)

        for i in range(self.block_seq_stride):
            write_pos = torch.full((self.bs,), write_seq_length + i, dtype=torch.int64)
