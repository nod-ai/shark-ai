    ...
    with open(log_file_path, 'r') as file:
        for line in file:
            if "GOLD PREFILL_TIME" in line:
                gold_prefill_time = float(line.split(':')[3].strip().split(' ')[0].replace(" ", ""))
                current_prefill_time = float(line.split(':')[4].strip().split(' ')[0].replace(" ", ""))
                print(gold_prefill_time, " ", current_prefill_time)
            if "GOLD DECODE_TIME" in line:
                gold_decode_time = float(line.split(':')[3].strip().split(' ')[0].replace(" ", ""))
                current_decode_time = float(line.split(':')[4].strip().split(' ')[0].replace(" ", ""))
                print(gold_decode_time, " ", current_decode_time)
    return gold_prefill_time, current_prefill_time, gold_decode_time, current_decode_time
...
def update_json_for_conditions(json_file_path, log_path):
    ...
    for model, details in data.items():
        log_file_path = os.path.join(log_path, f"output_{model}/e2e_testing_log_file.log")

        if not os.path.exists(log_file_path):
            continue

        gold_prefill, current_prefill, gold_decode, current_decode = parse_log(log_file_path)
