...
class BrevitasQuantizationTest(unittest.TestCase):
    ...
    def test_linear_layer_quantization_functional(self):
        ...
        for module in quantized_model.modules():
            if isinstance(module, brevitas.nn.QuantLinear):
                ...
                break
        self.assertTrue(
            found_quant_layer, "Expected to find qnn.QuantLinear in quantized model"
        )
    ...
    def test_linear_quantization_accuracy(self, input_size, output_size, with_bias):
        ...
        torch_linear.weight.data = (
            shark_linear.theta.tensor("weight").as_torch().clone()
        )
        if with_bias:
            torch_linear.bias.data = (
                shark_linear.theta.tensor("bias").as_torch().clone()
            )
    ...
    def test_shark_sequential_model_quantization(self):
        ...
        self.assertEqual(
            quant_layer_count,
            2,
            "Expected to find 2 qnn.QuantLinear layers in quantized model",
        )

        # Verify quantization preserves reasonable accuracy
        torch.testing.assert_close(
            reference_output, quantized_output, atol=0.2, rtol=0.1
        )
    ...
    def test_quantization_layer_integration(self):
        ...
        quantizer = StaticScaledQuantizer(
            scale=scale, dtype=torch.float8_e4m3fnuz, name="quantizer"
        )
        ...
        for module in quantized_model.modules():
            if isinstance(module, brevitas.nn.QuantIdentity):
                ...
                break
        self.assertTrue(
            found_quant_identity,
            "Expected to find qnn.QuantIdentity in quantized model",
        )
        ...
        torch.testing.assert_close(
            reference_output.unpack().dequant(), brevitas_output, atol=0.2, rtol=0.1
        )
