...
def compute_perplexity(token_ids: torch.tensor, logits: torch.tensor, start: int) -> list[float]:
      ...
      attention_mask = (
            (token_ids != 0).int().detach().clone().to(token_ids.device)
        )
      ...
      assert token_ids.shape == logits.shape[0:2], "Mismatching token_ids and logits shapes, verify bs, seq_len dims match"

      loss_fct = torch.nn.CrossEntropyLoss(reduction="none")

      ## perplexity = e ^ (sum(losses) / num_tokenized_tokens)
      crossentropy_loss = (
            loss_fct(logits.transpose(1, 2), token_ids)
            * attention_mask
      ).sum(1)
      
      perplexity_batch = torch.exp(
            crossentropy_loss / attention_mask.sum(1)
      ).tolist()
