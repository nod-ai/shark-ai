...
def create_theta(
    ...
    sharded_gate_weight = SplitPrimitiveTensor(
        name="ffn_gate.weight", shard_dim=0, ts=gate_weight.split(split_size, dim=0), devices=tuple(range(shard_count)), devices_pinned=True
    )
    sharded_up_weight = SplitPrimitiveTensor(
        name="ffn_up.weight", shard_dim=0, ts=up_weight.split(split_size, dim=0), devices=tuple(range(shard_count)), devices_pinned=True
    )
    ...
    sharded_down_weight = SplitPrimitiveTensor(
        name="ffn_down.weight", shard_dim=1, ts=down_weight.split(split_size, dim=1), devices=tuple(range(shard_count)), devices_pinned=True
    )
