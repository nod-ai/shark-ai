...
def apply_rope(xq: Tensor, xk: Tensor, freqs_cis: Tensor) -> tuple[Tensor, Tensor]:
    xq = xq.permute(0, 2, 1, 3).to(freqs_cis.dtype)
    xk = xk.permute(0, 2, 1, 3).to(freqs_cis.dtype)
    xq_out = kernels.apply_rotary_embedding(xq, freqs_cis)
    xk_out = kernels.apply_rotary_embedding(xk, freqs_cis)
