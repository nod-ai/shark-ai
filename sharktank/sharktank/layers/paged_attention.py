...
__all__ = ["PagedAttention", "attn_type_map", "get_kv_pages"]
...
def get_kv_pages(page_ids, positions, block_seq_stride) -> AnyTensor:
    page_index = positions // block_seq_stride
    write_page_ids = ops.gather(page_ids, dim=1, index=page_index.unsqueeze(1)).view(
        (positions.shape[0], 1, 1)
    )
    return write_page_ids
