# Modified from ``https://github.com/openai/CLIP'' and ``https://github.com/mlfoundations/open_clip'' and transformers.models.xlm_roberta.modeling_xlm_roberta
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = ops.scaled_dot_product_attention(
            q, k, v, a=None, is_causal=self.causal, impl="decomposed"
        )
        x = x.reshape(b, s, c)
...
class CLIPModel:
    def __init__(self, dtype, device, tokenizer_path):
        self.dtype = dtype
        self.device = device
