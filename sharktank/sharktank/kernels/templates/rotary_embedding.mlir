...
util.func private @sharktank_rotary_embedding_hf_{{bs}}_{{sl}}_{{heads}}_{{dims}}_{{dtype}}(%input: !input_tensor_type, %table: !table_tensor_type) -> !input_tensor_type {
  ...
  %d3_half = arith.divui %d3, %c2 : index
  ...
  %result = linalg.generic {
    ...
    ^bb0(%b0 : {{dtype}} , %b1 : {{dtype}}):
      ...
      %2 = linalg.index 2 : index
      %3 = linalg.index 3 : index
      %div = arith.divui %3, %d3_half : index
      %mod = arith.remui %3, %d3_half : index
      %a_cosb = math.cos %b0 : {{dtype}}
      %a_sinb = math.sin %b0 : {{dtype}}
      %imag_index = arith.addi %mod, %d3_half : index
      %real = tensor.extract %input[%0, %1, %2, %mod] : !input_tensor_type
      %imag = tensor.extract %input[%0, %1, %2, %imag_index] : !input_tensor_type
      %cmp = arith.cmpi eq, %div, %c0 : index
      %real_t0 = arith.mulf %real, %a_cosb : {{dtype}}
      %real_t1 = arith.mulf %imag, %a_sinb : {{dtype}}
      %real_t2 = arith.subf %real_t0, %real_t1 : {{dtype}}
      %imag_t0 = arith.mulf %imag, %a_cosb : {{dtype}}
