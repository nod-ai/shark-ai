...
TEST_CASE("Convolution fprop X (NCHW) W (KCRS)", "[conv][graph]") {
  int64_t n = 16, c = 128, h = 64, w = 64, k = 256, r = 1, s = 1;

  auto build_new_graph = [=](const Handle &handle) {
    auto graph = std::make_shared<Graph>();
    graph->setName("conv_fprop_sample_nchw_kcrs");
...
TEST_CASE("Convolution fprop X (NHWC) W (KRSC)", "[conv][graph]") {
  int64_t n = 16, c = 128, h = 64, w = 64, k = 256, r = 1, s = 1;

  auto build_new_graph = [=](const Handle &handle) {
    auto graph = std::make_shared<Graph>();
    graph->setName("conv_fprop_sample_nhwc_krsc");
    graph->setIODataType(DataType::Half).setComputeDataType(DataType::Float);
