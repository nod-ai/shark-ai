...
public:
  ...
  error_t validate() const {
    FUSILI_RETURN_ERROR_IF(dim_.empty(), error_code_t::ATTRIBUTE_NOT_SET,
                           "Tensor '" + name_ + "' dims not set");
    FUSILI_RETURN_ERROR_IF(stride_.empty(), error_code_t::ATTRIBUTE_NOT_SET,
                           "Tensor '" + name_ + "' strides not set");
    FUSILI_RETURN_ERROR_IF(
        dim_.size() != stride_.size(), error_code_t::INVALID_ATTRIBUTE,
        "Tensor '" + name_ +
            "' uses dim and stride of different dimensionality");

    FUSILI_RETURN_ERROR_IF(
        isVirtual_ && isScalar_, error_code_t::INVALID_ATTRIBUTE,
        "Tensor '" + name_ +
            "' cannot be both virtual (intermediate) and a scalar constant");

    FUSILI_RETURN_ERROR_IF(
        scalarValue_.has_value() && !isScalar_, error_code_t::INVALID_ATTRIBUTE,
        "Tensor '" + name_ +
