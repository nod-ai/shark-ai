...
// TORCH-CHECK:     func.func @main(%result_: !torch.tensor<[16,256,64,32],f32>, %arg0_image: !torch.vtensor<[16,128,64,32],f32>, %arg1_filter: !torch.vtensor<[256,16,1,1],f32>) attributes {torch.assume_strict_symbolic_shapes} {
...
// LINALG-CHECK:      %[[BUF1E:.+]] = tensor.expand_shape %[[BUF1]] {{\[\[0\], \[1, 2\], \[3\], \[4\]\]}} output_shape [16, 8, 16, 64, 32] : tensor<16x128x64x32xf32> into tensor<16x8x16x64x32xf32>
...
// LINALG-CHECK:      %{{.+}} = hal.tensor.alias wait(%{{.+}}) => %[[OUTC]] : tensor<16x256x64x32xf32> to %[[ARG0]] : !hal.buffer_view
//
...
ErrorObject
test_conv_grouped_asm_emitter_x_nchw_w_kcrs(const std::string &mode) {
  int64_t n = 16, c = 128, h = 64, w = 32, k = 256, fc = 16, r = 1, s = 1;
  auto graph = std::make_shared<Graph>();
  graph->setName("conv_grouped_asm_emitter_x_nchw_w_kcrs");
...
int main(int argc, char **argv) {
  ...
  auto status = test_conv_grouped_asm_emitter_x_nchw_w_kcrs(mode);
  if (isError(status)) {
